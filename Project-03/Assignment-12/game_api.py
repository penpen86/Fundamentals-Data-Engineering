...
def log_to_kafka(topic, event):
    ...
    event.update({'remote_addr': request.remote_addr}) # We augment the event with the actual remote address so that we can handle remote IP addresses
...
def purchase_sword():
    ...
    if request.method == 'GET':
        ...
        if request.headers['Content-Type'] == 'application/json':
            sword_event = {'event_type': 'purchase_a_sword:', 'attributes': json.dumps(request.json)}
            log_to_kafka('userItems', sword_event)
...
def get_coins():
    ...
    if request.method == 'GET':
        ...
        if request.headers['Content-Type'] == 'application/json':  
            get_coins_event = {'event_type': 'get_coins', 'attributes': json.dumps(request.json)}
            log_to_kafka('userItems', get_coins_event)
